...
from _sjmin99_model.BaseModel import BaseModel
...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(netD_input_nc, opt.train_ndf, opt.train_n_layers_D, opt.norm, use_sigmoid,
        ...
        if self.isTrain:
            ...
            self.loss_filter = self.init_loss_filter(not opt.train_no_gan_loss, not opt.train_no_ganFeat_loss, not opt.train_no_vgg_loss,
                                                     not opt.train_no_l1_loss)
            self.criterionGAN = networks.GANLoss(use_lsgan=not opt.train_no_lsgan, tensor=self.Tensor)
